    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (player == null) {
                ...
                commandSender.sendMessage(MessageUtil.message(SimplyStaff.getInstance().getConfig().getString("COMMANDS.Basic.teleport.message"), player));
                StaffAlert.alert(SimplyStaff.getInstance().getConfig().getString("COMMANDS.Basic.teleport.alert").replaceAll("%staff%", commandSender.getName()), player);
